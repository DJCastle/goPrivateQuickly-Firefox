...
// setting builder and the Firefox adapter decision logic. Browser-integration
// behavior (window lifecycle) is covered by the manual testing checklist.

import { test } from "node:test";
import assert from "node:assert/strict";

import { buildHardenedSet } from "../src/shared/hardenedDefaults.js";
import { firefoxAdapter } from "../src/shared/firefoxAdapter.js";
...
test("firefox: every setting is reported unavailable (no global mutation)", async () => {
  const r = await firefoxAdapter.applyOne(WEBRTC_ENTRY);
